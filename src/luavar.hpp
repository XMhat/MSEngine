...
{ /* -- Private variables -------------------------------------------------- */
  ...
  { // This is stop a false positive in CppCheck about cviVar not being const
    // because it HAS to be non-const as some other cvar callbacks may need to
    // directly modify the value. Compiler will just optimise it out without
    // complaining.
    cviVar.NullOp();
  ...
  CVarSetEnums Set(const string &strValue) const
    { return cCVars->Set(lcvmiIt->second.second,
        strValue, PUSR, CCF_SAFECALL); }
  ...
  { // Must be running on the main thread
    ...
    lcvmiIt->second.second = cCVars->RegisterVar(strName,
